    ...
    public partial class WheelController : WheelUAPI
    {
        ...
        protected virtual void UpdateFriction()
        {
            ...
            for (int substep = 0; substep < frictionSubsteps; substep++)
            {
                ...
                if (_isGrounded)
                {
                    // Recalculate forward speed each substep to account for coordinate frame changes during steering
                    float currentForwardSpeed = Vector3.Dot(_hitContactVelocity, _wheelForward);
                    float currentAbsForwardSpeed = currentForwardSpeed < 0 ? -currentForwardSpeed : currentForwardSpeed;
                    float currentClampedSpeed = currentAbsForwardSpeed < forwardSpeedClamp ? forwardSpeedClamp : currentAbsForwardSpeed;
                    float currentInvClampedSpeed = 1.0f / currentClampedSpeed;

                    forwardFriction.slip = -(wheel.angularVelocity * wheel.radius - currentForwardSpeed) * currentInvClampedSpeed * forwardFriction.stiffness;
            ...
            // Recalculate one final time for the slip output value
            float finalForwardSpeed = Vector3.Dot(_hitContactVelocity, _wheelForward);
            float finalAbsForwardSpeed = finalForwardSpeed < 0 ? -finalForwardSpeed : finalForwardSpeed;
            float finalClampedSpeed = finalAbsForwardSpeed < forwardSpeedClamp ? forwardSpeedClamp : finalAbsForwardSpeed;
            float finalInvClampedSpeed = 1.0f / finalClampedSpeed;

            forwardFriction.slip = _isGrounded ?
                -(wheel.angularVelocity * wheel.radius - finalForwardSpeed) * finalInvClampedSpeed * forwardFriction.stiffness : 0.0f;
